...
def generate_comparison_report(source_dir, destination_dir, comparison_file):
    ...
    with open(comparison_file, "w", encoding='utf-8') as f:
        f.write(generate_html_template(
            title="[Dated Backup]Comparison Report",
            src_count=src_count,
            src_size=src_size,
            dst_count=dst_count,
            dst_size=dst_size,
...
def generate_reports(source_dir, destination_dir):
    ...
    generate_file_list(source_dir, SRC_FILE, f"[Dated Backup]Files in {os.path.basename(source_dir)}")
    generate_file_list(destination_dir, DST_FILE, f"[Dated Backup]Files in {os.path.basename(destination_dir)}")
